...
def choose_input():
    ...
    try:
        inputfile = filedialog.askopenfile(
            initialdir="/", title="Zvol soubor k zašifrování", filetypes=(("documents", "*.docx"), ("text files", "*.txt")))
...
def encode():
    ...
    try:
        ...
        raise Exception(messagebox.showerror(
            "Error", "Je potřeba vybrat steganografickou metodu."))
    ...
    try:
        ...
        raise Exception(messagebox.showerror(
            "Error", "Musíš zvolit vstupní soubor."))
    if method == 0:
        method_name = '-b'
    elif method == 1:
        method_name = '-w'
    ...
    try:
        check = main(['-i', str(filepath), '-e',
                     '-s', secret_mes, method_name])
    except:
        raise Exception(messagebox.showerror(
            "ERROR!", "Chybně zadané parametry! (Baconova šifra beze pouze znaky A-Z (bez mezer a speciálních znaků), totéž platí pro metodu synonym vužívající tohoto šifrování"))
    if check is False:
        messagebox.showerror(
            "ERROR!", "Soubor nemá dostatečnou kapacitu na ukrytí této zprávy.")
    else:
        messagebox.showinfo(
            "ENCODED!", "Zpráva byla úspěšně zašifrována do zvoleného souboru!")
...
def decode():
    ...
    try:
        ...
        raise Exception(messagebox.showerror(
            "Error", "Nutno zlovit vstupní soubor."))
    ...
    if method == 0:
        ...
        try:
            ...
            raise Exception(messagebox.showerror(
                "ERROR!", "Dešifrování Huffmanova kódování nebylo implementováno. Huffmanův strom musí být vložen do textu, nebo domluven mezi komunikujícími stranami předem."))
...
heading = Label(text="STEGOSHARK 2000", bg="black",
                fg="white", height="3", width="800")
heading.pack()

l1 = Label(text="Vstupní soubor:", background="#DCDFE0")
l1.place(x=20, y=90)
# input file button
open_file = tk.Button(root, text="Vybrat vstupní soubor", padx=5, pady=5, bg="#4173EA",
                      fg="#FFFFFF", activebackground="#1C3A83", activeforeground="#FFFFFF", command=choose_input)
open_file.place(x=140, y=80)

l2 = Label(text="Vlož tajnou zprávu, kterou si přeješ ukrýt:",
           background="#DCDFE0")
l2.place(x=20, y=140)
...
# encode button
encode_button = Button(root, text="ŠIFROVAT", command=encode, padx=85, bg="#4173EA",
                       fg="#FFFFFF", activebackground="#1C3A83", activeforeground="#FFFFFF")
encode_button.place(x=60, y=450)

decode_button = Button(root, text="DEŠIFROVAT", command=decode, padx=85, bg="#4173EA",
                       fg="#FFFFFF", activebackground="#1C3A83", activeforeground="#FFFFFF")
...
combo['values'] = ["Baconova šifra", "Open-space metoda", "Metoda synonym",
                   "Metoda synonym s baconovým šifrováním", "Huffmanovo kódování"]
