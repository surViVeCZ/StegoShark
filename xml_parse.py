    ...
    def split_document(self) -> str:
        ...
        for par in tree.iter(paragraph_tag):
            ...
            for run in par.findall(run_tag):  # <w:p>-><w:r>
                run_props_q_res = run.findall(
                    run_properties)  # <w:p>-><w:r>-><w:rPr>

                run_props = None if len(
                    run_props_q_res) == 0 else run_props_q_res[0]
                ...
                if (self.method == "bacon"):
                    ...
                    for word in words:
                        ...
                        new_run = new_run_element(
                            word, bit, run_props, self.method)
        ...
        workaround_tree.write(
            "document.xml", encoding='UTF-8', xml_declaration=True)
...
def new_run_element(word: str, bit: int, properties_to_inherit: str, method: str) -> str:
    ...
    prop_el = xml.etree.ElementTree.Element(
        run_properties) if properties_to_inherit is None else deepcopy(properties_to_inherit)

    # přidání stylů do xml souboru docx,
    # odstranění původních (těch, které způsobují kolizi)
    if (method == "bacon"):
