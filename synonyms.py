...
# returns <w:rStyle w:val="skipthis"/>
def create_syn_tag():
...
def syn_element(prop_el,bit,namespace, word):
    word = word.lower()
   
    if(word in dictionary_of_zeros):
        print("index in dict: %d (%s)" % (list(dictionary_of_zeros.keys()).index(word), word))
        #TODO funkce na prohození slov slovníků    
